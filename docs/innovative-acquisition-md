...
**Guidance text that can be displayed as an info button:**

Process innovation concerns how the service is produced and delivered:

- **Production methods:** New ways of creating the solution. For example, introducing automated case handling using algorithms to replace manual assessments, or welfare technology that changes how care is provided.

- **Delivery methods:** New ways of reaching the user. For example, moving from physical service offices to digital self-service solutions.
...
Product innovation concerns what the user receives: the end result that meets the citizen or user.

- **New services:** Services that did not previously exist. For example, a new low-threshold mental health service without referral.

- **Significantly improved services:** Existing services with new qualities that change the value for the user. For example, waste collection using sensor-based emptying when needed, instead of fixed collection days.
