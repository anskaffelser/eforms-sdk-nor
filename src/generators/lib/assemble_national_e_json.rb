...
fragment_files.each do |path|
  ...
  # Finn ut hvilken logikk som skal brukes basert på filendelsen
  # Vi sjekker hvilken nøkkel i MERGE_LOGIC som finnes i filnavnet
  ...
  if action
    ...
    puts "  ⚠️  [SKIP]   Unknown fragment type (check naming convention): #{filename}"
  end
end
...
puts "📊 Stats: #{out['fields'].size} fields, #{out['rules'].size} rule blocks."
puts "------------------------------------------------------------"
